...
import { Skia, BlendMode, ColorType, AlphaType } from '@shopify/react-native-skia';
  ...
  runOnRuntime(getExportRuntime(), () => {
    ...
    try {
      try {
        ...
        const nbFrames = Math.ceil(videoComposition.duration * options.frameRate);
        let lastProgressAt = -Infinity;
        const progressInterval = Number.isFinite(progressIntervalMs) ? Math.max(0, progressIntervalMs) : 100;
        ...
        for (let i = 0; i < nbFrames; i++) {
          ...
          runPooled(() => {
            ...
            const now = Date.now();
            if (onProgress && (i === nbFrames - 1 || now - lastProgressAt >= progressInterval)) {
              lastProgressAt = now;
