...
| `encoderMode` | `'copy' \| 'direct'` | iOS only. `copy` (default) copies each rendered frame through a CPU readable texture. `direct` blits it once on the GPU straight into the encoder's buffer: one full frame copy and one GPU wait less per frame. |
...
:::info Color
The exported file declares **Rec.709 SDR**, which is what the frames coming out
of Skia are. HDR sources are tone-mapped at decode, so the export matches the
preview instead of shifting in whichever player opens it.
:::
