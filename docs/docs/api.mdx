...
| `textureMode` | `'copy' \| 'direct'?` | iOS: how this item's frames reach Skia (see [video player](/docs/video-player#options)). Default `copy`. |
...
| `encoderMode` | `'copy' \| 'direct'?` | `copy` | iOS: how rendered frames reach the encoder (see [exporting](/docs/video-composition/exporting)). |
